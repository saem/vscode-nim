...
proc createNimModule(projectDir:cstring, rootDir:cstring, dir:cstring, file:cstring):NimModuleInfo =
    ...
    if nimModules[projectDir].toJs().to(bool):
        for nimModule in nimModules[projectDir]:
            if not prefix.isNil() or nimModule.name.startsWith(prefix):
                var suggest = vscode.newCompletionItem(nimModule.name, VscodeCompletionKind.module)
